document.addEventListener("DOMContentLoaded", function () {
    ...
    // اگر عدد خیلی نزدیک به یک عدد صحیح است
...
function convertParenthesesDivision(expression) {
    ...
    return expression.replace(
        ...
        function(match, inside, denominator) {
            ...
            const multiplier =
                Math.round((1 / d) * 1000000) / 1000000;
...
constant = Math.round(constant * 1000000) / 1000000;
xCoefficient = Math.round(xCoefficient * 1000000) / 1000000;
xSquaredCoefficient = Math.round(xSquaredCoefficient * 1000000) / 1000000;
...
function createLinearSteps(result, expandedInfo) {
    ...
    */
    ...
    if (b !== 1) {
        ...
        const rightValue =
    Math.abs((result.x * b) - Math.round(result.x * b)) < 0.00001
        ? Math.round(result.x * b)
        : result.x * b;
